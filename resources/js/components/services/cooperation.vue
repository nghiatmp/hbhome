    ...
    <v-card class="pa-3 ma-3" style="box-shadow: 0 0px 0px 0px rgba(0,0,0,.2), 0 0px 2px 0 rgba(0,0,0,.14), 0 1px 5px 0 rgba(0,0,0,.12);">
      <v-row>
        <v-col cols="12" md="3" sm="12">
          <v-select
            v-model="params.cooperation_service_id"
            :items="services"
            label="サービスID"
        ...
        <v-col cols="12" md="3" sm="12">
          <v-select
            v-model="params.cooperation_time_group_by"
            :items="TIME_GROUP_BY"
            item-value="key"
            item-text="value"
            label="グループ化"
        ...
        <v-col cols="12" md="3" sm="12">
          ...
          >
            <template v-slot:activator="{ on }">
              <v-text-field
                v-model="params.cooperation_start_at"
                label="開始日"
                prepend-icon="event"
                :hide-details="true"
                clearable
            ...
            <v-date-picker v-model="params.cooperation_start_at" :max="nowDate" @input="menuFrom=false" />
          </v-menu>
        </v-col>
        <v-col cols="12" md="3" sm="12">
          <v-menu
          ...
          >
            <template v-slot:activator="{ on }">
              <v-text-field
                v-model="params.cooperation_end_at"
                label="終了日"
                prepend-icon="event"
                :hide-details="true"
                clearable
            ...
            <v-date-picker v-model="params.cooperation_end_at" :min="params.cooperation_start_at" :max="nowDate" @input="menuTo=false" />
          </v-menu>
        </v-col>
      </v-row>
    </v-card>
...
export default {
  name: 'ServiceCooperationComponent',
  components: {
    highcharts: Chart,
  },
  data() {
    return {
      ...
      params: {
        cooperation_service_id: '',
        cooperation_time_group_by: TIME_GROUP_BY_DEFAULT,
  ...
  methods: {
    renderData() {
      ...
      if (this.params.cooperation_service_id) {
        ...
        this.axios
          .get(`service/${this.params.cooperation_service_id}/cooperation`, { params })
          .then(res => {
            ...
            this.tableData = Object.keys(resData).map((key) => {
              const tableDataItem = { ... resData[key], duration: key };
    ...
    index() {
      ...
      this.axios
        ...
        .then(res => {
          ...
          if (paramSearch.cooperation_service_id.length === 0 && this.services[0]) {
            paramSearch.cooperation_service_id = this.services[0].service_id;
          }
